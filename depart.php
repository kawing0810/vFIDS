...
<table class="table <?php echo $default_lang; ?>" id="table" cellspacing="0" cellpadding="0">
    <thead>
        <tr>
            ...
            <th class="lang" data-lang="DESTINATION">Destination</th>
    ...
    function showTable(tbody) {
        ...
        for(var idx=spos; idx < epos; idx++) {
            ...
            if(row.status=='ARRIVED') {
                ...
                // tdStatus.addClass('departed');
            } else if(row.status=='DELAY') {
                // tdStatus.addClass('delay');
            } else {
                tdStatus.html('');
                $('<span/>', {class:'en', html:gLang['en']['ESTAT']}).appendTo(tdStatus);
                $('<span/>', {class:'zh', html:gLang['zh']['ESTAT']}).appendTo(tdStatus);
                if ( typeof row.eta != 'undefined' && row.eta.length == 4 ) {
                    $('<div/>', {class:'time', html:row.eta.substr(0,2) +":"+ row.eta.substr(2,2)}).appendTo(tdStatus);
                }
            }
